...
class settingController extends Controller
{
    ...
    public function airports(Request $request){
        try{
            $rows1 = DB::table('airport_details')->orderBy('id','desc')->paginate(30);
            return view('settings.airports',['airports' => $rows1]);
        }
        catch(\Illuminate\Database\QueryException $ex){
            return back()->with('errorMessage', $ex->getMessage());
        }
    ...
    {
        $airport = DB::table('airport_details')->where('id', $id)->first();

        return view('settings.editAirport', compact('airport'));
    }
    public function updateAirport(Request $request)
    {
        ...
        DB::table('airport_details')
         ...
         return redirect()->to('airports')->with('successMessage', 'Airport updated successfully!');
    }
