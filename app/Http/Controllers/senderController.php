...
class senderController extends Controller
{
    ...
    {
        return view('sender.emailSender');
    }
    public function sendEmail(Request $request)
    {
        ...
        if ($request->hasFile('attachments')) {
            foreach ($request->file('attachments') as $file) {
                ...
                $file->move(public_path('email_attachments'), $fileName);
                $attachmentPaths[] = 'email_attachments/'.$fileName;
            }
        }

        // Single / Multiple
        if (in_array($request->email_type, ['single', 'multiple'])) {
            $emails = array_map('trim', explode(',', $request->emails));
        }

        // Excel Import (Your Style)
        if ($request->email_type == 'excel') {
            ...
            foreach ($rows as $index => $row) {
                // If header exists, uncomment ↓
                // if ($index == 0) continue;

                $excelEmail = isset($row[0]) ? trim($row[0]) : null;
        ...
        if (count($emails) == 0) {
            return back()->with('errorMessage', 'No valid email found!');
        }
        ...
        foreach ($emails as $email) {
            DB::table('email_queue')->insert([
                ...
                'attachment' => json_encode($attachmentPaths),
                'agent_id'   => $agent_id,
                'status'     => 0,
                'created_at' => now(),
                'updated_at' => now(),
        ...
        return back()->with('successMessage', 'Emails added to queue successfully!');
    }
    public function emailLog(Request $request)
    {
        ...
        return view('sender.email-log', compact('logs'));
    }

    // 🔄 Retry Failed Email
    public function retryEmail($id)
    {
        $log = DB::table('email_queue')->where('id', $id)->first();

        if (!$log || $log->status == 1) {
            return back()->with('errorMessage', 'Email already delivered or not found');
        }

        $company = DB::table('users')->where('id', $log->agent_id)->first();
        ...
        return back()->with('successMessage', 'Retry request sent!');
    }

    // ❌ Delete Log
    public function deleteEmailLog($id)
    {
        DB::table('email_queue')->where('id', $id)->delete();
        return back()->with('successMessage', 'Log deleted!');
    }
    ...
    {
        $agent_id = Session::get('agent_id');
        $logs = DB::table('email_queue')->where('agent_id', $agent_id)->get();
        ...
        $row = 2;
        foreach ($logs as $log) {
            $sheet->setCellValue("A$row", $log->email);
            $sheet->setCellValue("B$row", $log->subject);
            $sheet->setCellValue("C$row", $log->status ? 'Delivered' : 'Pending');
            $sheet->setCellValue("D$row", $log->created_at);
            $row++;
        }

        $writer = new Xlsx($spreadsheet);
        $filename = "email_logs.xlsx";
    ...
    public function exportPDF()
    {
        ...
        $company = DB::table('users')->where('id', $agent_id)->first();

        $pdf = Pdf::loadView('sender.email-log-pdf', compact('logs','company'))
