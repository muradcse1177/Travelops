...
class visaController extends Controller
{
    ...
    public function editVisa(Request $request)
    {
        try {
            ...
            if (!$visa) {
                return back()->with('errorMessage', 'Visa record not found.');
            }
            $existingClientFiles = json_decode($visa->payment_files, true) ?? [];
            $existingVendorFiles = json_decode($visa->vendor_payment_files, true) ?? [];
