...
class hotelController extends Controller
{
    public function hotelBooking(Request $request)
    {
        try {
            ...
            $query = DB::table('hotel_invoice')
                ->where('agent_id', Session::get('agent_id'))
                ->orderBy('id', 'desc');

            // ⭐ Filter: From Date
            if ($request->from_date) {
                $query->whereDate('b_date', '>=', $request->from_date);
            }

            // ⭐ Filter: To Date
            if ($request->to_date) {
                $query->whereDate('b_date', '<=', $request->to_date);
            }

            // ⭐ Filter: Reservation No
            if ($request->reservation) {
                $query->where('reservation', 'LIKE', "%{$request->reservation}%");
            }
            ...
            if ($request->hotel_name) {
                $query->where('h_name', 'LIKE', "%{$request->hotel_name}%");
            }

            // ⭐ Filter: Payment Status (Paid / Due)
            ...
            // ⭐ Pagination Support
            $rows1 = $query->paginate(20)->appends($request->all());

            // Extra Data
            $rows2 = DB::table('payment_type')->get();
            $rows3 = DB::table('passengers')->where('upload_by', Session::get('agent_id'))->get();
