                            ...
                            <div class="card-body">
                                @if(Session::get('user_role') <= 2)
                                    <form method="GET" action="{{ url()->current() }}" class="mb-3">
                                        <div class="form-row">
                                            <div class="col-md-6">
                                                <select name="employee_id" class="form-control select2bs4" onchange="this.form.submit()">
                                            ...
                                            @if(!empty($employeeId))
                                                <div class="col-md-2 mt-2 mt-md-0">
                                                    <a href="{{ url()->current() }}" class="btn btn-secondary btn-block">Reset</a>
                                                </div>
                                            @endif
                                        </div>
                                    </form>
                                    {{-- ===== Bootstrap-only Dashboard Cards (no external CSS) ===== --}}
                                    @if(!empty($employeeId) && !empty($selectedEmployee))
                                        @php
                                            $usedByCat = $usedByCat ?? [];

                                            // label => [employee_column, category_key_in_DB]
                                            $defs = [
                                                'Casual'     => ['casual_leave','Casual'],
                                                'Sick'       => ['seek_leave','Sick'],          // DB-তে যদি 'Seek' থাকে, 'Sick' -> 'Seek' দিন
                                                'Marriage'   => ['marriage_leave','Marriage'],
                                                'Fatherhood' => ['fatherhood','Fatherhood'],
                                                'Motherhood' => ['motherhood','Motherhood'],
                                                'Earned'     => ['earned_leave','Earned'],
                                            ];

                                            // Bootstrap রঙ ও soft-bg (inline rgba) ম্যাপ
                                            $palette = [
                                                'Casual'     => ['text' => 'text-primary', 'bg' => 'bg-primary', 'soft' => 'rgba(13,110,253,.12)'],
                                                'Sick'       => ['text' => 'text-danger',  'bg' => 'bg-danger',  'soft' => 'rgba(220,53,69,.12)'],
                                                'Marriage'   => ['text' => 'text-warning', 'bg' => 'bg-warning', 'soft' => 'rgba(255,193,7,.20)'],
                                                'Fatherhood' => ['text' => 'text-info',    'bg' => 'bg-info',    'soft' => 'rgba(23,162,184,.18)'],
                                                // Bootstrap-এ pink নেই; টেক্সট ও সফট রঙ ইনলাইন কোডে দিলাম
                                                'Motherhood' => ['text' => '',             'bg' => '',           'soft' => 'rgba(232,62,140,.16)', 'hex' => '#e83e8c'],
                                                'Earned'     => ['text' => 'text-success', 'bg' => 'bg-success', 'soft' => 'rgba(40,167,69,.16)'],
                                            ];

                                            $icons = [
                                                'Casual'     => 'fas fa-user-clock',
                                                'Sick'       => 'fas fa-briefcase-medical',
                                                'Marriage'   => 'fas fa-ring',
                                                'Fatherhood' => 'fas fa-baby',
                                                'Motherhood' => 'fas fa-baby-carriage',
                                                'Earned'     => 'fas fa-coins',
                                            ];

                                            $allotted = [
                                                'Casual'     => (int)($selectedEmployee->casual_leave ?? 0),
                                                'Sick'       => (int)($selectedEmployee->seek_leave ?? 0),
                                                'Marriage'   => (int)($selectedEmployee->marriage_leave ?? 0),
                                                'Fatherhood' => (int)($selectedEmployee->fatherhood ?? 0),
                                                'Motherhood' => (int)($selectedEmployee->motherhood ?? 0),
                                                'Earned'     => (int)($selectedEmployee->earned_leave ?? 0),
                                            ];
                                            $used = [];
                                            foreach ($defs as $label => [$col, $cat]) $used[$label] = (int)($usedByCat[$cat] ?? 0);

                                            $showRemaining = $showRemaining ?? true;
                                            $values = [];
                                            foreach ($allotted as $label => $val) $values[$label] = $showRemaining ? max($val - ($used[$label] ?? 0), 0) : $val;
                                        @endphp

                                        <div class="container-fluid px-0">
                                            <div class="row">
                                                @foreach($defs as $label => [$col, $cat])
                                                    @php
                                                        $pal   = $palette[$label];
                                                        $icon  = $icons[$label];
                                                        $vAll  = $allotted[$label] ?? 0;
                                                        $vUsed = $used[$label] ?? 0;
                                                        $vShow = $values[$label] ?? 0;
                                                        $pct   = $vAll > 0 ? min(100, round(($vUsed / $vAll) * 100)) : 0;

                                                        // রঙ ক্লাস/ইনলাইন ক্যাল্ক
                                                        $textClass = $pal['text'] ?? '';
                                                        $bgClass   = $pal['bg'] ?? 'bg-secondary';
                                                        $softBg    = $pal['soft'] ?? 'rgba(108,117,125,.15)';
                                                        $hexColor  = $pal['hex'] ?? ''; // Motherhood only
                                                    @endphp
                                                        ...
                                                        <div class="card shadow-sm rounded-lg h-100 border-0"
                                                            ...
                                                            <div class="card-body d-flex align-items-center p-3">
                                                                <div class="d-flex align-items-center justify-content-center rounded"
                                                                     style="width:56px;height:56px;background: {{ $softBg }};">
                                                                    <i class="{{ $icon }} {{ $textClass }}" style="font-size:20px;{{ $hexColor ? 'color:'.$hexColor.';' : '' }}"></i>
                                                                </div>

                                                                <div class="ml-3 flex-fill">
                                                                    <div class="text-secondary small mb-1">{{ $label }}</div>
                                                                    <div class="font-weight-bold h6 mb-1">{{ $vShow }} Days</div>
                                                                    <div class="text-muted small mb-2">
                                                                        Used: <span class="text-dark font-weight-bold">{{ $vUsed }}</span>
                                                                        &nbsp;&nbsp; Allotted: <span class="text-dark font-weight-bold">{{ $vAll }}</span>
                                                                    </div>
                                    ...
                                    <hr>
                                @endif
                                <div class="row"> @php $i = 1; @endphp @foreach($leaves as $leave) @php $user = DB::table('users')->where('id', $leave->user_id)->first(); $employee = DB::table('employees')->where('email', @$user->company_email)->first(); @endphp <div class="col-12 mb-3">
                                        <div class="card shadow border-left border-info">
                                            <div class="card-header bg-info text-white d-flex justify-content-between align-items-center py-2 px-3">
                                                <h6 class="mb-0">
                                                    <i class="fas fa-calendar-check mr-2"></i>Leave Request #{{ $i }} - <b>{{@$employee->name}}</b>
                                                </h6>
                                                <span class="badge
                                            ...
                                            <div class="card-body">
                                                <div class="row align-items-start">
                                                    ...
                                                    <div class="col-md-4 col-sm-12 mb-3">
                                                        <h6 class="text-info">
                                                            <i class="fas fa-user mr-1"></i> {{ @$user->company_name }}
                                                        </h6>
                                                    ...
                                                    <div class="col-md-4 col-sm-12 mb-3">
                                                        <h6>
                                                            <i class="fas fa-info-circle text-muted mr-1"></i> Reason
                                                        </h6>
                                                        <p class="text-muted small mb-0">{{ json_decode($leave->cause) }}</p>
                                                    </div>
                                                    <!-- Leave Balances + Action --> @if(Session::get('user_role') <= 2) <div class="col-md-4 col-sm-12">
                                                        <div class="row"> @php $leaveTypes = [ ['label' => 'Casual', 'val' => @$employee->casual_leave,
                                                         'color' => 'primary', 'icon' => 'user-clock'], ['label' => 'Sick', 'val' => @$employee->seek_leave, 'color' => 'danger',
                                                          'icon' => 'briefcase-medical'], ['label' => 'Marriage', 'val' => @$employee->marriage_leave, 
                                                          'color' => 'warning', 'icon' => 'ring'], ['label' => 'Fatherhood', 'val' => @$employee->fatherhood, 
                                                          'color' => 'info', 'icon' => 'baby'], ['label' => 'Motherhood', 'val' => @$employee->motherhood,
                                                           'color' => 'secondary', 'icon' => 'baby-carriage'], ['label' => 'Earned', 'val' => @$employee->earned_leave,
                                                            'color' => 'success', 'icon' => 'coins'], ]; @endphp <div class="col-12 mb-2">
                                                                ...
                                                                <div class="btn-group">
                                                                    ...
                                                                    <div class="dropdown-menu dropdown-menu-right">
                                                                        <a class="dropdown-item" href="{{ url('approveLeave?id=' . $leave->id) }}">
                                                                            <i class="fas fa-check-circle text-success mr-1"></i> Approve </a>
                                                                        <a class="dropdown-item" href="{{ url('rejectLeave?id=' . $leave->id) }}">
                                                                            <i class="fas fa-times-circle text-danger mr-1"></i> Reject </a>
                                                                        <a class="dropdown-item" href="{{ url('approveEarnedLeave?id=' . $leave->id) }}">
                                                                            <i class="fas fa-coins text-warning mr-1"></i> Approve Earned Leave </a>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
